...
import {getProjectsByParams, addModelIdProject} from './../../../../actions/n2sky/project-actions'
...
export default class CopyModelToProjectPopup extends React.Component {
	...
	submitForm() {
		this.commit()
			.then(r => {
				this.props.dispatch(addModelIdProject(r.project, r.model_id)).then(() => {
					location.reload();
				});
			}).catch(err => {
	...
	commit() {
		return new Promise((resolve, reject) => {
			...
			let obj = {
				model_id: {model_id: this.props.model._id},
	...
	render() {
		return (<div>
			...
			<OpenStackMonitoringModal showCloseModal={this.props.showCloseModal}
																...
																title="Copy model to your project"/>
		</div>)
	}
}
